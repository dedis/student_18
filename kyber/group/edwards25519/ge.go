...
// b for G and h for Point
//func selectDoubleCached(c *cachedGroupElement, Ai, Gi *[8]cachedGroupElement, a,g int32) {
//
//	aNegative := negative(a)
//	aAbs := a - (((-aNegative) & a) << 1)
//	gNegative := negative(g)
//	gAbs := a - (((-gNegative) & g) << 1)
//
//	// in constant-time pick cached multiplier for exponent 0 through 8
//	c.Zero()
//	for i := int32(0); i < 8; i++ {
//		c.CMove(&Ai[i], equal(aAbs, i+1))
//		c.CMove(&Gi[i], equal(gAbs, i+1))
//	}
//
//	// in constant-time compute negated version, conditionally use it
//	var minusC cachedGroupElement
...
func geDoubleScalarMult(h *extendedGroupElement, a *[32]byte,
	...
	for i, v := range a {
		eA[2*i] = int8(v & 15)
		eA[2*i+1] = int8((v >> 4) & 15)
	}
	var eG [64]int8
	for i, v := range b {
		eG[2*i] = int8(v & 15)
		eG[2*i+1] = int8((v >> 4) & 15)
	}
	// each e[i] is between 0 and 15 and e[63] is between 0 and 7.
