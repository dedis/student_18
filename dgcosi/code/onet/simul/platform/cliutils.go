...
func SSHRun(username, host, command string) ([]byte, error) {
	...
	// FIXME hardcoded key for windows
	//log.LLvl1("******** K cliutils_L61: specific key addr")
	keyAddr := "D:/Code/Keys/deter"
	log.Lvl4("Going to ssh to", addr, command)
	args := [10]string{  "-o", "StrictHostKeyChecking=no", addr,
		"eval '"+command+"'"}
	if runtime.GOOS == "windows" {
		args[4] = "-i"
		args[5] = keyAddr
		log.Lvl3("**** K specific key address from windows")
